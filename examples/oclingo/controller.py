...
# Parse Command Line Options
    ...
    parser.set_defaults(**contrDefaults)
    return parser
...
class GlobalData:
    ...
    def __init__(self):
        self.args = Struct(**contrDefaults)
        
    def initClArg(self):
        self.args = getClParser().parse_args()
        self.initCoParser(self.args.copred)
       # print "bbsdfs" + self.args.copred
        
    def initCoParser(self, copred):
        global CO_PARSER
        CO_PARSER = [
            re.compile(copred + "(\((.*?),?(\d+)?\))?")
        ]
...
additionsEnc= "% General Additions\n" # generic additions for the oclingo program
currentIncStep = 0
exit = False
    ...
    while not exit:
        try:
            try:
                ...
                print e.globalData.args.files[0]
                continue
            ...
            if globalData.args.wait == "yes":  # if synchronous execution, fetch next input
                input = getInput()
                sendInput(s, input)
            else:
...
def addQatomEquival():
    ...
    global equivalEnc
    for q in globalData.args.qquads:
        if QQUAD.match(q):
        ...
        for pred in answer_set:
            for i in range(len(CO_PARSER)):
                ...
                if match != None:
                    ...
                    if globalData.args.debug: print "found " + pred + "\n"
        if globalData.args.debug: print "CO list is " + ' '.join(co_input)

                    
def connectToSocket(s, host, port):
    try:
        s.connect((host, port))
        # s.setblocking(0)
    except socket.error:
        raise EnvironmentError("Could not connect to %s:%s" % (host, port))
    return

# Prepares input stream for processing if given as file
def prepareInput():
    """Prepares input stream for processing if given as file."""
    if len(globalData.args.files) == 1:
        file = globalData.args.files[0]
    ...
    elif len(globalData.args.files) > 1:
...
def recv_until(s, delimiter):
    ...
    if len(data_list) > 1:
        ...
        while not exit:
            ...
            if globalData.args.timeout: s.settimeout(globalData.args.time)
...
def getInput():
    ...
    if len(globalData.args.files) == 1:
        if not globalData.args.timeout:
            time.sleep(float(globalData.args.time))
           ...
           INTEGRITY.match(line) != None or\
            ...
            if globalData.args.ainc:
                ...
                # Standard volatile query
                    ...
                    if globalData.args.iWindowSize:
                        volatileDirective = "#volatile: " + str(globalData.args.qWindowSize) + ".\n"
                    #no volatility
                    elif globalData.args.novol:
                        volatileDirective = ""
...
def sendInput(s, input):
    if globalData.args.debug:
        print "Sending following input to oclingo:"
        print input
...
def launchOclingoWrapper(enc, addenc):
    global additionsEnc
    """Launches oclingo wrapper for given encoding"""
    if not enc == '':
        globalData.args.oclenc = enc
        if addenc:
            additionsEnc += addenc
        threado = OclingoThread()
        threado.start()
...
class OclingoThread(Thread):
    global equivalEnc, additionsEnc
    def __init__ (self):
        Thread.__init__(self)
    def run(self):
        # start oclingo
        # print "echo " + " \'" + equivalEnc + "\' | cat " +
        # args.oclenc[0] + " - | oclingo "
        invocation =([("echo " + " \'" + equivalEnc + "\n" + additionsEnc +
                       "\' | cat " + globalData.args.oclenc[0] + " - | oclingo")] +
                     globalData.args.oclpar
                     )
        if globalData.args.debug:
            print "Oclingo Invocation: \n" + ''.join(invocation) +"\n"
            
        call(invocation, shell=True)
        return
