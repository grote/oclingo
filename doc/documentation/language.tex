...
by atoms other than assignments mentioning~\var{X} on their right-hand side.
...
As with built-in comparison functions,
...
the rule can be simplified to a fact \code{\pred{on\_week}.}
(as done by \gringo).
\eexample
\end{example}
...
First note that \pred{mix}/$2$ and \pred{-mix}/$2$ each admit four options,
...
The (repeated) body also contains two instance each of \pred{sym}/$1$ and of \pred{num}/$1$.
...
\end{lstlisting}
%
Note that courses~\const{4} and~\const{5} count three times because
...
\code{\const{course}(\var{5},\var{S},\const{4})}, respectively.
...
\code{\#domain} declaratives (\code{\#}~is optional)
