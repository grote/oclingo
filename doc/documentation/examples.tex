...
Thus, the instance contains six nodes and 17 directed edges among them.
...
over predicates \pred{block}/$0$ (declaring blocks),
\pred{init}/$1$ (defining the initial state),
...
Our Blocks-World Planning encoding for \iclingo\ makes use of statements
...
at step~\code{t-1}.
Finally, the Define part in Line~15 propagates
...
must not occur in the cumulative part,
and those of the cumulative part must in turn not be used in the static part.
...
This is granted for our encoding because both atoms over \pred{move}/$3$
and those over \pred{holds}/$2$ include~\const{t} as an argument in the
heads of the rules in Line~9, 15, and~16--17.
Notably, the smallest step number to replace~\const{t} with is~\const{1},
so that there is no clash with the ground atoms over \pred{holds}/$2$
obtained from the head of the static rule in Line~5.
Further details on these requirements and their formal background can
be found in~\cite{gekakaosscth08a}.
Arguably, many problems including some mutable bound are naturally encoded
such that the above conditions apply.
...
stops its incremental computation after finding some answer set in step~\const{3}.
