...
\marginlabel{%
  ...
  \code{-flies(X)} in order to make explicit that
  instances amount to the classical (or strong) negation of
  \code{flies(X)}.}
%
...
\footnotetext{Described in this document.}
...
\lstinputlisting[firstnumber=3]{examples/bfall.lp}
%
...
%
\lstinputlisting[nolol]{examples/bird.lp}
\lstinputlisting[firstnumber=3]{examples/bfeff.lp}
%
...
the rule with \code{not \pred{neg\mus flies}(\const{tux})} in the body has been dropped completely.
...
the body a (possibly empty) conjunction of atoms that may be preceded by ``\code{not}.''
