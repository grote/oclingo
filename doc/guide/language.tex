...
for an atom~$A$ where the connective \code{not} corresponds to default negation.
...
all variables that appear in a rule have to appear in some some positive literal (a literal not preceded by \code{not}) in the body.
...
\code{\#sum} (the sum of weights), 
\code{\#min} (the minimum weight), 
\code{\#max} (the maximum weight), and
\code{\#avg} (the average of all weights
...
r(X) :- q(X), p(X).
\end{lstlisting}
Here, the external predicate \code{q}/$1$ will not be used for simplification
...
The values in this table are then successively replaced for the call \code{\@rng{X,Y}}.
