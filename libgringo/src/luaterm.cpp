...
void LuaTerm::normalize(Lit *parent, const Ref &ref, Grounder *g, const Expander &e, bool /*unify*/)
{
	for(TermPtrVec::iterator it = args_.begin(); it != args_.end(); it++)
	{
		it->normalize(parent, Term::VecRef(args_, it), g, e, false);
	}
	uint32_t var = g->createVar();
	e(new LuaLit(loc(), new VarTerm(loc(), var), args_, name_, g->luaIndex(loc(), name_)), Lit::RANGE);
