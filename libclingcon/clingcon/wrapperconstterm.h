...
namespace Clingcon
{
	class WrapperConstTerm : public WrapperTerm
	{
	public:
		WrapperConstTerm(const Loc &loc, const Val &v) : WrapperTerm(loc), v_(v)
		{
		}

		ConstraintConstTerm* toConstraintTerm()
		{
			return new ConstraintConstTerm(loc_,v_);
		}

		ConstTerm* toTerm() const
		{
			return new ConstTerm(loc_, v_);
		}

		WrapperConstTerm *clone() const
		{
			return new WrapperConstTerm(loc_,v_);
		}
	private:
		Val v_;
	};
